...
import { followModel, unfollowModel } from '@/lib/follows/actions'
...
export function FollowButton({
  ...
  function handleClick() {
    ...
    startTransition(async () => {
      ...
      const result = wasFollowing
        ? await unfollowModel(modelId)
        : await followModel(modelId)

      if (!result.error) {
        // Pulls fresh server data so the optimistic value commits to the
        // new baseState instead of reverting when the transition ends.
        router.refresh()
      }
    })
  }
  ...
  return (
    <button
      ...
      aria-label={
        state.isFollowing ? `Unfollow ${modelName}` : `Follow ${modelName}`
      }
      className={cn(
